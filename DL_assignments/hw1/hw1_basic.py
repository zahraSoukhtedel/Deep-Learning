...
def multiples_of_ten(start, stop):
  ...
  if start > stop:
    ...
    x = torch.arange(start=start, end=stop, step=10, dtype=torch.float64)
...
def reverse_rows(x):
  ...
  """
  y = x[[i for i in range(x.shape[0]-1, -1, -1)], :].clone()
  return y
...
def make_one_hot(x):
  ...
  y[[i for i in range(len(x))], x] = torch.ones(len(x))
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    z = torch.stack([x[i, :, :].mm(y[i, :, :]) for i in range(x.shape[0])], dim=0)
  else:
    z = torch.bmm(x, y)
  return z
...
def normalize_columns(x):
  ...
  """
  y = x.clone()
  y = y - ((torch.ones(1, x.shape[0])).mm(x))/x.shape[0]
  std = ((torch.ones(1, x.shape[0])).mm(y.clone() ** 2)/(x.shape[0] - 1)) ** 0.5
  y = y / std
  return y
